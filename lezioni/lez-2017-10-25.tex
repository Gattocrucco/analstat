...
\marginpar{Riformulare questa roba come:
definizione di massima likelihood;
teorema che è consistente e asintoticamente gaussiano.}%
...
\emph{Controesempio alla sufficienza:
prendiamo la combinazione di due gaussiane con media diversa
$p(x;q)=qg(x;0,1)+(1-q)g(x;1,1)$, $q\in[0,1]$.
Per una estrazione, $\hat q\in\set{0,1}$
quindi non è possibile che $p(x;q)=f(x)g(\hat q,q)$.}}%
