...
\begin{definition}[Funzione generatrice]
	Data la pdf $p(x)$, la sua \emph{funzione generatrice} è
		$M(t) \is E[e^{tx}]$.
	\marginpar{Si chiama trasformata di Laplace?}
\end{definition}
...
\marginpar{Bisogna fare anche la multinomiale dopo la binomiale,
serve anche dopo per gli istogrammi.}
\begin{align*}
	P(n_1,n_2,\dotsc)
...
\marginpar{Falso, la generatrice dell'esponenziale $e^{-x}$ è $(1-t)^{-1}$ con dominio $t<1$.}
Esiste però~la
\begin{definition}[Funzione caratteristica]
	La \emph{funzione caratteristica} è la trasformata di Fourier della pdf:
	\marginpar{Spostare a dopo la generatrice.}
	\begin{equation*}
		\phi(t) \is E[e^{itx}].
	\end{equation*}
\end{definition}
