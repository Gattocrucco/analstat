...
\marginpar{L'implicazione si dimostra così: sia $A=x_k$, $B=x_{k-1}$,
$C=(x_1,\dots,x_{k-2})$. $p(A|B) = \frac{p(A,B)}{p(B)} = \frac{\sum_C p(A,B,C)}
{\sum_C p(B,C)} = \frac{\sum_C p(A,B,C)} {\sum_C p(A,B,C) / p(A|B,C)}$, ma
per ipotesi $p(A|B,C)$ non dipende da $C$, quindi le somme si semplificano e
viene $p(A|B) = p(A|B,C)$.}
